-- ===== ANIMATED AUTOMATIC GROUP-STAFF DETECTOR (NATIVE & FIXED) =====
...
-- HTTP Request für Server Hop (bleibt für Server Hop erhalten)
...
local function getUserDetails(userId)
    local details = { displayName = "N/A", hasVerifiedBadge = false, ageDays = "N/A" }
    local success, result = pcall(function()
        local info = Players:GetNameFromUserIdAsync(userId)
        return info
    end)
    return details
end
...
-- ===== PRÜFUNG DER SPIELER MIT NATIVEN FUNKTIONEN =====
local function checkPlayer(player, isStartup)
    ...
    for _, groupId in ipairs(trackedGroups) do
        ...
        if success and rank and rank > 0 then
            ...
            -- Prüfe ob die Rolle Keywords enthält oder der Rank hoch ist (z.B. >= 100)
...
-- ===== HAUPTABLAUF =====
task.spawn(function()
    -- Ermittle die Gruppen-ID des Spiels automatisch
    local success, info = pcall(function() return MarketplaceService:GetProductInfo(game.PlaceId) end)
    if success and info and info.Creator then
        if info.Creator.CreatorType == "Group" then
            table.insert(trackedGroups, info.Creator.CreatorTargetId)
            print("✅ Game Group ID automatisch gefunden: " .. tostring(info.Creator.CreatorTargetId))
        end
    end
    ...
    -- Test-Benachrichtigung für dich selbst beim Start (zeigt, dass das UI klappt)
    playAlertSound()
    createNotification({
